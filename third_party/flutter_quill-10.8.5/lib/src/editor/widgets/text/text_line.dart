...
class RenderEditableTextLine extends RenderEditableBox {
  ...
  void performLayout() {
    ...
    // LOCAL PATCH: bullet marker follows alignment.
    // Upstream pins the list leading to the line's left edge, so center/right-
    // aligning a list line moves the text but leaves the `•` far left. The
    // leading slot width equals the left padding, which is also where the body
    // starts; the bullet is right-aligned within that slot, so it normally sits
    // just left of the text. By shifting the slot right by the body's first-glyph
    // x (`startX`), the slot's right edge again meets the (now centered/right-
    // aligned) text start, keeping the same gap — the marker and text move as one
    // group. Left alignment has startX == 0 (no shift); justify and RTL keep the
    // upstream behavior.
    if (_leading != null && textDirection == TextDirection.ltr) {
      final align = line.style.attributes[Attribute.align.key]?.value;
      if (line.style.attributes.containsKey(Attribute.list.key) &&
          (align == 'center' || align == 'right')) {
        final startX = _body!
            .getOffsetForCaret(const TextPosition(offset: 0), _caretPrototype)
            .dx;
        if (startX > 0) {
          (_leading!.parentData as BoxParentData).offset =
              Offset(startX, _resolvedPadding!.top);
        }
      }
    }
  }
